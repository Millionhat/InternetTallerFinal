...
public class PersonControllerImp {
	...
	private IdDocTypeService idts;
	
	@Autowired
	public PersonControllerImp(InstitutionService serv, PersonService ser, DocumentService doc, IdDocTypeService idc) {
		ins=serv;
		per=ser;
		this.doc=doc;
		idts=idc;
	...
	public String addPer(Model model) {
		...
		model.addAttribute("types",idts.getIDTS());
	...
	public String editInfo(@PathVariable("id") long id, Model model) {
		final Optional<Person> person=per.getPerson(id);
		if(person== null) {
	...
	public String updatePerson(@Validated(update.class) @ModelAttribute Person person,
		...
		if(action!=null && !action.equals("Cancel")) {
			...
			try {
				person.setPersId(id);
				person.setPersIddocument(per.getPerson(id).get().getPersIddocument());
				per.addPerson(person);
			} catch (NonNullValueException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
	...
	public String deletePerson(@PathVariable("id") long id, Model model) {
		final Optional<Person> perse=per.getPerson(id);
		if(perse==null) {
			throw new IllegalArgumentException("Invalid Doc Type Id: "+id);
		}
		per.delete(id);
		model.addAttribute("poeple",per.getPeople());
		return "redirect:/person/";
	}
	...
	public String consultInfo(@PathVariable("id") long id, Model model) {
		...
		model.addAttribute("institutions",ins.getInstitutions());
		model.addAttribute("types",idts.getIDTS());
		model.addAttribute("documents",doc.findDocs(person.get()));
